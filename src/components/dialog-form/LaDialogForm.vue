<template>
  ...
  >
    ...
    >
      <template v-for="(item, idx) in slotNames" :key="idx" #[item.slotName]>
...
import { ref, reactive, defineProps, withDefaults, defineEmits, computed } from 'vue'
...
const labelWidth = computed(() => (props.labelWidth ? props.labelWidth : 'auto'))
const slotNames = computed(() => props.formRows.filter((item) => item.component === 'slot'))

const dialogFormRef = ref()

const handleBeforeClose = (done: () => void) => {
  ConfirmBox('是否需要关闭弹出框？')
    .then(() => {
      emits('before-close', false)
      setTimeout(() => emits('update:formData', {}), 400)
      done()
    })
    .catch(() => {})
}
